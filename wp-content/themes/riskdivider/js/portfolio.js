...
$(document).ready( function() { 
	$('body').addClass('min-h-[100vh] bg-gradient-to-br from-dark-slate-blue to-denim-blue to-[106%] bg-no-repeat');

	function max_drawdown(returns) {
		var max = Math.max(...returns);
	
		var min = Math.max(Math.min(...returns), 1);
	
		return (max/min)-1;
	}    
	
	function annualizedReturn(portfolioReturn, length) {
		return ((((portfolioReturn * 100) ^ (1/length))-1))/100;
	}
	
	function portfolioReturns(assets, optimal) {
		return assets.map((asset, index) => {
			let weight = optimal ? window.optimalWeights[index] : asset.weight;
	
			return asset.averageChange * weight;
		}).reduce((a, b) => {
			return a + b;
		});
	}
	...
	*/
	...
	Matrix_.prototype = {
		...
		*
		* @see <a href="https://mathworld.wolfram.com/NonpositiveMatrix.html"> Weisstein, Eric W. "Nonpositive Matrix." From MathWorld--A Wolfram Web Resource. https://mathworld.wolfram.com/NonpositiveMatrix.html </a>
		*
		* @memberof Matrix_
		...
		*
		* @see <a href="https://mathworld.wolfram.com/NegativeMatrix.html">Weisstein, Eric W. "Negative Matrix." From MathWorld--A Wolfram Web Resource. https://mathworld.wolfram.com/NegativeMatrix.html </a>
		*
		* @memberof Matrix_
	...
	* @see <a href="https://www.netlib.org/lapack/lawnspdf/lawn150.pdf">Anderson, Edward (4 December 2000). Discontinuous Plane Rotations and the Symmetric Eigenvalue Problem. LAPACK Working Note. University of Tennessee at Knoxville and Oak Ridge National Laboratory.</a>
	*
		...
		var methods = {
			...
			* either stored in the matrix out or in a new matrix.
			...
			'getVariancesVector': function(out) {
				...
				*
				* @see <a href="https://dx.doi.org/10.1016/0047-259X(87)90153-9">Ashis SenGupta, Tests for standardized generalized variances of multivariate normal populations of possibly different dimensions, Journal of Multivariate Analysis, Volume 23, Issue 2, 1987, Pages 209-219</a>
				*
				* @memberof Matrix_
	...
	* @see <a href="https://dl.acm.org/citation.cfm?doid=365719.365958">Peter M. Neely (1966) Comparison of several algorithms for computation of means, standard deviations and correlation coefficients. Commun ACM 9(7):496–499.</a>
	*
	...
	* @see <a href="https://dx.doi.org/10.1287/opre.23.3.581">Robert F. Love, James G. Morris, (1975) Technical Note—Solving Constrained Multi-Facility Location Problems Involving lp Distances Using Convex Programming. Operations Research 23(3):581-587.</a>
	...
	* @see <a href="https://ieeexplore.ieee.org/document/6789464/">S. S. Keerthi, S. K. Shevade, C. Bhattacharyya and K. R. K. Murthy, "Improvements to Platt's SMO Algorithm for SVM Classifier Design," in Neural Computation, vol. 13, no. 3, pp. 637-649, March 1 2001.</a>
	...
	*
	* @see <a href="https://ieeexplore.ieee.org/document/6126441/">T. Pock and A. Chambolle, "Diagonal preconditioning for first order primal-dual algorithms in convex optimization" 2011 International Conference on Computer Vision, Barcelona, 2011, pp. 1762-1769.</a>
	...
	* @see <a href="https://www.numerical.rl.ac.uk/reports/drRAL2001034.pdf">D. Ruiz ,A scaling algorithm to equilibrate both rows and column norms in matrices, Tech.Report RT/APO/01/4, ENSEEIHT-IRIT, 2001.</a>
	*
	...
	* @see <a href="https://www.sciencedirect.com/science/article/pii/S0377221707004262">De Klerk, E., Den Hertog, D., Elabwabi, G.: On the complexity of optimization over the standard simplex. Eur. J Oper. Res. 191, 773–785 (2008)</a>
	* @see <a href="https://ideas.repec.org/p/cor/louvco/2003071.html">Nesterov, Yurii. Random walk in a simplex and quadratic optimization over convex polytopes. CORE Discussion Papers ; 2003/71 (2003)</a>
	*
	...
	* @see <a href="https://www.hss.caltech.edu/content/subset-optimization-asset-allocation">Benjamin J. Gillen, Subset Optimization for Asset Allocation, SOCIAL SCIENCE WORKING PAPER 1421, June 1, 2016</a>
	* @see Harry M. Markowitz, Portfolio Selection, Efficient Diversification of Investments, Second edition, Blackwell Publishers Inc.
	...
	*
	* @see <a href="https://www.jstor.org/stable/2634472">Young, M. (1998). A Minimax Portfolio Selection Rule with Linear Programming Solution. Management Science, 44(5), 673-683.</a>
	...
	*/
	...
	(async function() {    
		assets = assets.map((asset) => {
			...
			window.portfolioValue += asset.quantity * asset.currentPrice;
	
			return asset;
		});
	
		for (let index = 0; index < assets.length; index++) {
			assets[index].weight = portfolioValue > 0 ? assets[index].marketValue / portfolioValue : 0;
			
			let current = assets[index].currentPrice;
			let initial = assets[index].closePrices[1];
	
			assets[index].averageChange = (current - initial) / initial;
		}
	
		// Optimal weights
		const returns = assets.map((asset) => {
			return asset.averageChange;
		});
	
		const dailyChanges = assets.map((asset) => {
			return asset.dailyChange;
		});
	
		covarianceMatrix = covariance([...dailyChanges]);
	
		JSON.parse(localStorage.investment)[3].value === 'growth' ? 
		window.optimalWeights = PortfolioAllocation.maximumSharpeRatioWeights(returns, covarianceMatrix, 0) :
		window.optimalWeights = PortfolioAllocation.globalMinimumVarianceWeights(covarianceMatrix);
		
		const optimalReturn = portfolioReturns(assets, true);
		const currentReturn = portfolioReturns(assets, false);
	
		let optimalReturns = assets.map((asset, index) => {
			let weight = window.optimalWeights[index];
	
			return asset.dailyChange.map((change) => {
				return change * weight;
			});
		});
	
		window.annualizedReturn = [annualizedReturn(optimalReturn, length), annualizedReturn(currentReturn, length)];
	
		var template2 = wp.template( 'assets-stats-template' );
	
		var statsReady = setInterval(() => {
			if (window.maxDrawdown && typeof window.annualizedReturn !== 'undefined') {
				clearInterval(statsReady);
	
				var stats = {
					optimal: {
						risk: window.maxDrawdown[0] < .05 ? 'Low' : (window.maxDrawdown[0] >= .05 && window.maxDrawdown[0] < .15) ? 'Average' : 'High',
						annualReturn: (window.annualizedReturn[0] * 100).toFixed(2) + '%',
						drawdown: (window.maxDrawdown[0] * 100).toFixed(2) + '%',
					},
					current: {
						risk: window.maxDrawdown[1] < .05 ? 'Low' : (window.maxDrawdown[1] >= .05 && window.maxDrawdown[1] < .15) ? 'Average' : 'High',
						annualReturn: (window.annualizedReturn[1] * 100).toFixed(2) + '%',
						drawdown: (window.maxDrawdown[1] * 100).toFixed(2) + '%',
					}
				}
			
				var html = template2( { stats } );
			
				$('.assets-stats').html(html); 
			}
		}, 500);	
	...
	$(document).on('click', '.shares-switch', function() {
		$('.shares, .percent').toggleClass('hidden');
 });

	$(document).on('click', '#chart1Legend .label', function() {
		$('.swatch').toggleClass('active');
	});

	$(document).on('click', '#chart1Legend .line', function() {
			$('.stats-current, .stats-optimal').toggleClass('hidden');
	});
  ...
  const ready = setInterval(function() {
      if (typeof window.assets !== 'undefined') {
        clearInterval(ready);
				...
				var template = wp.template( 'assets-table-template' );
        var content = assets.map(function(asset) {
      ...
      $('.assets-table').html(html); 


			// Pie chart
			var selectedAssets = assets.map((item) => {
				return {
					label: item.symbol,
			...
			const colorScheme = ['#fcd97e', '#53a691', '#f27979', '#495dc6', '#dd0074', '#ff8d00', '#00ACC1', '#E53935', '#8E24AA', '#039BE5', '#00897B', '#C0CA33', '#7CB342', '#43A047', '#FDD835', '#FFB300', '#F4511E', '#546E7A', '#6D4C41', '#FFB300'];
			
			renderPieChart(selectedAssets.map((asset) => {
  ...
  let seriesB = [];
	...
	const optimizationReady = setInterval(() => {
		if (window.optimalWeights && window.optimalWeights.length > 0 ) {
			clearInterval(optimizationReady);

			// Current portfolio accumulated returns
	let length;
	
	for (let index = 0; index < assets.length-1; index++) {
		let asset = assets[index];
		let next = assets[index+1].dailyChange.length;
		let current = asset.dailyChange.length;
		length = Math.min(next, current);
	}

	let currentPortfolioAccumulatedReturns = [];
	let optimalPortfolioAccumulatedReturns = [];
	...
	for (let index = 0; index < length; index++) {
		...
		for (let j = 0; j < assets.length; j++) {
			let asset = assets[j];
			let dailyChange = asset.dailyChange[index];
			let weight = asset.weight;
			let optimalWeight = window.optimalWeights[j];

			optimal += ((dailyChange) * optimalWeight );
			current += ((dailyChange) * weight );
		}

		optimalPortfolioAccumulatedReturns.push(optimal);
		currentPortfolioAccumulatedReturns.push(current);
	}

	window.maxDrawdown = [max_drawdown(optimalPortfolioAccumulatedReturns), max_drawdown(currentPortfolioAccumulatedReturns)];

			assets.map((asset, index) => {
        asset.closePrices.map((price, x) => {
          Ay = currentPortfolioAccumulatedReturns;
          By = optimalPortfolioAccumulatedReturns;
          
          seriesA[x] = {
            x, y: Ay[x] * price, change: Ay[x]
          }

          seriesB[x] = {
            x, y: By[x] * price, change: By[x]
          }
      ...
      var graph = new Rickshaw.Graph({
        ...
        series: [{
          ...
          color: '#495dc6',
          className: 'chart1-optimal',
          name: 'Portfolio otimizado',
          data: seriesB
        }],
      });
      ...
      var Hover = Rickshaw.Class.create(Rickshaw.Graph.HoverDetail, {
        render: function(args) {
          ...
          details.style.top = args.mouseY + "rem";
          ...
          args.detail.sort(function(a, b) {
            ...
            let ePercent = document.createElement('td');
            
            ePercent.className = 'expected-change';
            ePercent.innerHTML = (((d.value.change) - 1) * 100).toFixed(2) + '%';
            
            let eValue = document.createElement('td');
            
            eValue.className = 'expected-value';
            eValue.innerHTML = ((d.value.y / d.series.data[i+1].y - 1) * 100).toFixed(2) + '%';
            ...
            dot.style.top = graph.y(d.value.y0 + d.value.y) + 'rem';
      ...
      const xAxis = new Rickshaw.Graph.Axis.X({
        graph,
        tickFormat: (x) => {
          var date = new Date(Date.parse(new Date()) - (seriesB.length) * 86400000 + (x * 86400000));
      ...
      graph.render();
			...
			$('#chart1Legend .label').first().prev().addClass('active');

			assets.forEach((asset, index) => {
				comparisonData[1].push({
					name: 'Current portfolio',
					axis: asset.symbol,
					value: asset.weight,
					percent: (asset.weight * 100).toFixed(2),
					shares: asset.quantity
				});
		
				comparisonData[0].push({
					name: 'Optimal portfolio',
					axis: asset.symbol,
					value: optimalWeights[index],
					percent: (optimalWeights[index] * 100).toFixed(2),
					shares: Math.floor(portfolioValue / asset.currentPrice * optimalWeights[index])
				});
			...
			const w = Math.max(260, $('.comparison-box').width() * 0.6) - margin.left - margin.right;
			const h = Math.max(260, $('.comparison-box').width() * 0.6);
		
			const data = comparisonData;
		
			const color = d3v4.scaleOrdinal().range(['#495dc6', "#fdd368"]);
			const opacity = d3v4.scaleOrdinal().range(['0.3', '1']);
			...
			// Comparison table
			var template3 = wp.template( 'comparison-table-template' );
			var html = template3( { optimal: comparisonData[0], current: comparisonData[1] } );
			$('.comparison-table').html(html); 

			// Action list
			const action = assets.map((asset, index) => {
				let action = optimalWeights[index] >= asset.weight ? 'buy' : 'sell';

				return {
					symbol: asset.symbol,
					action: action,
					quantity: Math.abs(comparisonData[0][index].shares - comparisonData[1][index].shares),
					color: action === 'buy' ? 'bg-[#65ba8a]' : 'bg-[#cd6262]'
				}
			});

			const template4 = wp.template( 'action-template' );
			html = template4( { action } );
